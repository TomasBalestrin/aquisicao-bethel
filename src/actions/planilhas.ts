...
export async function duplicatePlanilha(
  ...
  const { data: nova, error } = await supabase.from("planilhas").insert({
    ...
    mes: parsed.data.mes,
    ano: parsed.data.ano,
    ob1_nome: original.ob1_nome,
    ob2_nome: original.ob2_nome,
    ob3_nome: original.ob3_nome,
    ob4_nome: original.ob4_nome,
    ob5_nome: original.ob5_nome,
    upsell_nome: original.upsell_nome,
    downsell_nome: original.downsell_nome,
  }).select("*").single();
