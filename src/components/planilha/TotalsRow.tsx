...
export function TotalsRow({ entries, columns }: Props) {
  return (
    <tr className="bg-gold-lightest font-semibold">
      {columns.map((col) => {
        if (col.key === "data") {
          return <td key={col.key} className="border-r border-gold-lighter bg-gold-lightest px-2 py-2 font-mono text-[12px] text-navy-dark">Total</td>;
        }
        const val = getTotal(entries, col.key);
        return (
          <td key={col.key} className={`border-r border-gold-lighter bg-gold-lightest px-2 py-2 font-mono text-[12px] tabular-nums text-navy-dark ${dyn(col.key, val)}`}>
            {fmt(val, col)}
          </td>
        );
      })}
